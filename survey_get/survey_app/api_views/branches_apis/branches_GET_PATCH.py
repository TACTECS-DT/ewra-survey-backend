...
class BranchPATCHGETAPIView(APIView):
    ...
    def patch(self, request, pk=None):
        ...
        try:
            ...
            instance = Branch.objects.select_related("affiliate").get(pk=pk)
            
        except Branch.DoesNotExist:
            raise ValidationError(f"Record with ID {pk} does not exist.")
        
        serializer = all_serializers.Branch_PATCH_POST_Serializer(instance, data=request.data, partial=True)
        ...
        if 'name' in request.data:
            ...
            if Branch.objects.filter(name=name_value).exclude(pk=pk).exists():
                raise ValidationError(f"The name '{name_value}' already exists. Please choose a unique name.")

        serializer.is_valid(raise_exception=True)
        instance = serializer.save()

        response_serializer = all_serializers.Branch_FORM_GET_Serializer(instance)
    ...
    def get(self, request, pk=None):
        ...
        try:
            ...
            record = Branch.objects.select_related("affiliate").get(pk=pk)
            
        except Branch.DoesNotExist:
            raise ValidationError(f"Record with ID {pk} does not exist.")

        response_serializer = all_serializers.Branch_FORM_GET_Serializer(record)
        return Response(response_serializer.data, status=status.HTTP_200_OK)
