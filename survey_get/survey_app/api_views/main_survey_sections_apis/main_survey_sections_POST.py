...
class MainSurveySectionPOSTAPIView(APIView):
    ...
    def post(self, request):
        ...
        serializer = all_serializers.MainSurveySection_PATCH_POST_Serializer(data=request.data)
        
        # Check if the name already exists in the database
        name_value = request.data.get('name').strip()
        if all_models.MainSurveySection.objects.filter(name=name_value).exists():
